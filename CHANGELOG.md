...
### Changed
- **Telemetry storage moved from JSON files to SQLite.** `telemetry.php` used to read, modify and write `telemetry_data.json` without any locking, so concurrent requests silently lost events, and `recent_logs` only ever kept the last 500 entries. Events now go into a `telemetry.sqlite` outside the webroot (a `.sqlite` in the webroot would be served as a static download) with WAL mode and a transaction around the rate-limit check and the insert. The old JSON counters are imported once and their totals preserved via an offset, and the JSON files stay on disk as an archive. The endpoint remains compatible with older clients that only send `?action=install|login`.
