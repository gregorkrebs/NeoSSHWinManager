...
& $pyinstaller --noconfirm --onefile --windowed --name "NeoSSHWinManager" --icon "assets/app_icon.ico" --add-data "src;src" --add-data "assets;assets" --version-file "file_version_info.txt" --hidden-import "PyQt6.sip" --hidden-import "win32api" --hidden-import "win32con" --hidden-import "winreg" --hidden-import "keyring" --hidden-import "keyring.backends.Windows" --noupx main.py
